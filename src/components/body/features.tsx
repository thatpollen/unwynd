    ...
    async function translateContent() {
      if (language !== "en") {
        const titles = await translateTexts(
          defaultItems.current.map((item) => item.title),
          language
        );
        const subtitles = await translateTexts(
          defaultItems.current.map((item) => item.subtitle),
          language
        );
        const descriptions = await Promise.all(
          defaultItems.current.map((item) =>
            translateTexts(item.description, language)
          )
        );

        setCarouselItems(
          defaultItems.current.map((item, index) => ({
            ...item,
            title: titles[index],
        ...
        setCarouselItems(defaultItems.current);
      }
    }
